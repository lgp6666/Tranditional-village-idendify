...
from Model.Public_Modules.HAT_SCConv import modify_model_ConvNeXt
...
def train(device, num_classes, train_data_path, val_data_path,
          epochs, batch_size, opt_name, nw, model_name):
    ...
    with open('CSV\\' + model_name + '_' + str(time_dir) + '_metrics.csv', mode='w', newline='') as file:
        ...
        pre_model_path = config['project_path'] + '/Config/pretrained/convnext_tiny_pre.pth'
        model.load_state_dict(torch.load(pre_model_path, map_location=torch.device(device)))
        ...
        model = modify_model_ConvNeXt(model)
...
if __name__ == '__main__':
    ...
    train(device, num_classes, train_data_path, val_data_path,
          epochs, batch_size, opt_name, nw, model_name)
